...
(provider, rerank, storage — flags > `LORE_*` > config file > defaults).
...
The natural workflow ties to **portable corpora**: build a collection, `lore
export` it (optionally encrypted), hand it off, `lore import` it on the agent's
host, and point `lore mcp` at it — a pre-indexed knowledge artifact an agent can
query and cite, no re-indexing required.
